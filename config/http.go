...
func init() {
	...
	config.Add("http", map[string]any{
		...
		// HTTP Host - use 0.0.0.0 for containerized environments like Railway
		"host": config.Env("HOST", config.Env("APP_HOST", "0.0.0.0")),
		// HTTP Port - respect Railway's PORT env var
		"port": config.Env("PORT", config.Env("APP_PORT", "3000")),
		// HTTP Timeout, default is 3 seconds (changed to 300 for AI tasks)
